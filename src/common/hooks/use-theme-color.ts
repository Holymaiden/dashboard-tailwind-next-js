...
function getLocalStoragePreset() {
  if (typeof window !== "undefined") {
    const localStorageValue = localStorage.getItem("fihdash-preset");
...
const colorPresetsAtomWithPersistence = atom(
  ...
  (get, set, newStorage: any) => {
    ...
    localStorage.setItem("fihdash-preset", JSON.stringify(newStorage));
  }
);
...
const colorPresetNameAtom = atom(
  typeof window !== "undefined"
    ? localStorage.getItem("fihdash-preset-name")
    : DEFAULT_PRESET_COLOR_NAME
);
...
const colorPresetNameAtomWithPersistence = atom(
  ...
  (get, set, newStorage: any) => {
    ...
    localStorage.setItem("fihdash-preset-name", newStorage);
  }
);
